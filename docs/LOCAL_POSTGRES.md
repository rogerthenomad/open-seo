# Running Run Your SEO on Postgres locally

Run Your SEO runs on **Cloudflare D1 (SQLite) by default**. Postgres is an opt-in
...
```sh
docker run --name runyourseo-postgres \
  -e POSTGRES_USER=runyourseo \
  -e POSTGRES_PASSWORD=runyourseo \
  -e POSTGRES_DB=runyourseo \
  -p 5433:5432 \
  -d postgres:16
```
...
```sh
docker exec runyourseo-postgres pg_isready -U runyourseo -d runyourseo
```

The connection string is:

```
postgres://runyourseo:runyourseo@localhost:5433/runyourseo
```
...
```sh
POSTGRES_DATABASE_URL=postgres://runyourseo:runyourseo@localhost:5433/runyourseo \
  pnpm db:migrate:pg
```
...
docker exec runyourseo-postgres psql -U runyourseo -d runyourseo -c "\dt"

# Inspect rows the app writes (e.g. after creating a project / saving keywords)
docker exec runyourseo-postgres psql -U runyourseo -d runyourseo -c "select count(*) from projects;"
```
...
```sh
docker rm -f runyourseo-postgres
```
