...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo
   ```

2. If the output looks right, apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo --apply --confirm-remote-apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo --validate-only
   ```
...
1. Preview the cleanup:
   ...
   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo --local
   ```

2. Apply it:

   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo --local --apply
   ```

3. Validate or re-run validation:

   ```sh
   pnpm cleanup:default-projects:d1 --database run-your-seo --local --validate-only
   ```
