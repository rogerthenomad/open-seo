...
Connecting Google Search Console (GSC) lets Run Your SEO pull your real clicks,
impressions, positions, and URL inspection data, straight from Google.

It's **optional**: Run Your SEO runs fine without it, just without Search Console data.
...
Set these three values, then restart Run Your SEO:
...
Restart Run Your SEO so it picks up the new variables. For Docker, changing `.env`
...
- Run Your SEO uses your Google client to run the OAuth flow and stores the resulting
...
- Search Console data comes from your own Google account, so Run Your SEO never meters credits for it.
