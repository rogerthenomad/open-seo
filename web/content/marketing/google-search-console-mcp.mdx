...
Any other MCP client works the same way: add `https://seo-app.capturethatmedia.com/mcp` as a remote server and sign in. Full setup and troubleshooting live in the [MCP docs](/docs/mcp).
...
The Search Console tools use zero credits — Google doesn't charge you to read your own data, so reading it through Run Your SEO never burns usage. They're included with the Run Your SEO plan, alongside the credit-based tools (keyword research, rank tracking, backlinks) that run through the same MCP endpoint.
...
The Search Console MCP tools are included with the Run Your SEO plan and use zero credits. Run Your SEO's other tools (keyword research, rank tracking, backlinks) use the usage credits included with the plan, through the same endpoint. For plan details, talk to Capture That Media.
