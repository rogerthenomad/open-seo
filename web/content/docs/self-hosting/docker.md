...
description: "Run Run Your SEO locally with Docker Compose using the published GHCR image."
---

Run Run Your SEO locally with Docker.

In Docker mode, Run Your SEO uses `AUTH_MODE=local_noauth` (no auth checks, local admin user `admin@localhost`). Only expose it behind your own auth-protected reverse proxy, tunnel, or private network. For internet-facing self-hosting, use [Cloudflare](/docs/self-hosting/cloudflare) instead.
